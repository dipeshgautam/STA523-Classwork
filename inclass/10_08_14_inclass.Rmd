...
State <- group_by(Christmas, Registration.State, DateTime)%>%
  summarize(totalViolations= n())%>%
  arrange(DateTime)
# plot(State$DateTime[State$Registration.State=="NY"], State$totalViolations[State$Registration.State=="NY"], type ="p", xlab="Date and time", ylab="Number of Violations")

#Group by day of week and find the day of most citations
day = group_by(StateDateTime, day=wday(DateTime))%>% summarize(totalViolations= n())
...
hourDay = group_by(StateDateTime, hour=hour(DateTime), day=wday(DateTime))%>% summarize(totalViolations= n())
