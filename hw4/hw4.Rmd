...
The code for `is_valid()` is commented so it should be clear what the goal is for each `if()` statement.

## `is_undirected()`
Checks that all directed edges have a complimentary directed edge with the same weight in the opposite direction.
...
“Be aware that there are established algortihms to solve most of the [predicate] problems ... don’t try to reinvent the wheel use what is known to work. Your task is translate the existing algorithms to our specific data structure and R.”
