...
if (i!=1){
  for (j in 1:32){
    rasterImage(alg, xleft=Countries$lon[j]-2,xright=Countries$lon[j]+4, ybottom=Countries$lat[j]-2, ytop=Countries$lat[j]+3)
    text(Countries$lon[j],Countries$lat[j], Countries$name[j], cex=.6, font=2)
  }
}
}
