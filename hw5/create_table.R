...
getTable= function(name, func){
  ...
  samples <- c(100, 10000, 1000000, 10000000)
  ...
  for (i in 1:length(samples)) {
    for (j in 1:length(dfunc)) {
      table[j,1]=dfunc[[j]]
      table[j, 2+i] = (system.time(func(samples[i],dfunc[[j]], range[[j]], mc = FALSE ) )[3])/samples[i]
#       print(c(j,i))   
      
      ## Multiple cores
      table[j, 6+i]= (system.time(func(samples[i],eval(dfunc[[j]]), range[[j]] , mc = TRUE) )[3])/samples[i]
      if (i==3){
        ## Single Core
        x = func(samples[i],eval(dfunc[[j]]), range[[j]], mc = FALSE )
        table[j, 11] = score(x,get(dfunc[[j]]))
        
        ## Multi Cores
        x = func(samples[i],eval(dfunc[[j]]), range[[j]], mc = TRUE )
