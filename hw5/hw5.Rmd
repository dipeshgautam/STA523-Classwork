...
For our implementation of Metropolis-Hastings Sampler we used the Random normal walk. First we had to tune the variance of the proposal distribution to get the desired acceptance ratio between 0.25 and 0.4. To accomplish this, we started with variance of .75 and ran Metropolis-Hastings algorithm on a smaller number of samples. We calculated the acceptance ratio for given variance and if it was not within desired range, variance was tweaked by either scaling it up or down depending on whether the acceptance ratio was too hign or to low. After getting the derised acceptance ratio, we fixed our variance for the proposal distribution and ran Metropolis-Hastings algorithm for the given number of iterations.
