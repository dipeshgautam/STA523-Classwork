...
###Task 3: Translate
The original data frame built from the raw data of  `world_cup_goals_raw.txt` has been converted into valid, well-formatted JSON through the single line of code below. The output below, excluding the "##" beginning on every line, can be copied and pasted into the the JSON validator: http://jsonlint.com/ to prove it is valid JSON. Unfortunately, `sink()` doesn't work with markdown, so it is difficult to `sink()` the easily copied and pastable JSON output to a .txt file.


 ```{r toJSON with country to goals hierarchy}
# countriesList<-as.data.frame(unique(Data$team))
# colnames(countriesList)<- c("country")
# goalDetails<-rep(NA, 10)
# goal<-list(goalDetails)
# jsonList<-list(list( list(),list(),goal))
# 
# for (i in 1:32){
#   
#     subset <- Data[Data$team == as.character(countriesList$country[i]),]
#     goal<-rep( list(goalDetails), nrow(subset) )
#     jsonList[i]<-list(list(list(),list(),goal))
#     for (j in 1:nrow(subset)){
#         goal[[j]]<-list("name"=as.character(subset$name[j])
#                       ,"playerId"=as.character(subset$playerID[j])
#                       ,"goalType"=as.character(subset$goalType[j])
#                       ,"class"=as.character(subset$class[j])
#                       ,"time"=as.character(subset$time[j])
#                       ,"addTime"=as.character(subset$addTime[j])
#                       ,"vertLoc"=as.character(subset$vertLoc[j])
#                       ,"horiLoc"=as.character(subset$horiLoc[j])
#                       ,"pointInGoal"=as.character(subset$pointInGoal[j])
#                       ,"matchId"=as.character(subset$matchID[j]))
#     }
# 
#     jsonList[i] <- list(list("Country"=as.character(subset$team[1])
#                       ,"Flag"=as.character(subset$teamImg[1])
#                       , "Goals"=goal))
#     
#         
# }
# convertedJSON <- rjson::toJSON(jsonList)
# jsonlite::prettify(convertedJSON)
